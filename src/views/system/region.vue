    ...
    <el-card class="box-card">
      ...
      <template>
        <el-table
          ...
          empty-text="暫無數據"
        >
          <el-table-column label="序號" type="index" :index="getIndex" width="50"></el-table-column>
          <el-table-column
            prop="code"
            label="區域代碼"
            width="120"
...
export default {
  ...
  methods: {
    ...
    add: function () {
      this.$refs.addRuleFormRef.validate(async (valid) => {
        if (!valid) {
          ...
          if (res.success) {
            this.$message.success("區域新增成功");
            this.addRuleForm = {};
            await this.getregionList();
          } else {
            return this.$message.error("區域新增失敗:" + res.data.errorMsg);
          }
    ...
    //加載區域列表
    async getregionList() {
      const { data: res } = await this.$http.get("system/region/findByPage", {
